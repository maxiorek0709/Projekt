...
int main()
{
	...
	short os_x = konsola.dwMaximumWindowSize.X / 2;					//punkt pocz¹tkowy (szerokoœæ)
	short os_y = konsola.dwMaximumWindowSize.Y / 10;				//punkt pocz¹tkowy (wysokoœæ)
...
/***************************************************************************************/
	...
	cin >> znak_ascii;
		...
		{
		cin.clear();																											//sprawdzenie poprawnoœci, wykluczenie bia³ych znaków
		cin.ignore(1000, '\n');
	...
	cin >> rozmiar_figury;											// wczytanie rozmiarów figury
		...
		while (cin.fail() || rozmiar_figury < 4 || rozmiar_figury > os_x)					//sprawdzenie poprawnoœci
		{
		cin.clear();
		cin.ignore(1000, '\n');
...
do {
	...
	{			
		...
		case lewo: 
			{ if (os_x > rozmiar_figury)					// zabezpieczenie	
			{ os_x--; }										// przesuniêcie w lewo
			break; } 
        
		case prawo: 
			{ if (os_x < konsola.dwMaximumWindowSize.X-1)		//zabezpieczenie
			{ os_x++; }											// przesuniêcie w prawo
			break; } 
		
		
		case gora: 
			{ if (os_y>0)									// zabezpieczenie
			{ os_y--; }										// przesuniêcie do góry
			break; }					
        
		case dol: { os_y++; break; }						// przesuniêcie w dó³
		
		case Esc: { break; }								// wyjœcie z programu
	}				
...
/*********ustalenie wspó³rzêdnych i drukowanie***************/
